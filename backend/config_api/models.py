...
def terrestre_media_path(instance, filename):
    """Nome único (uuid) para imagens/vídeos do terrestre, preservando a extensão."""
    ext = filename.rsplit('.', 1)[-1].lower() if '.' in filename else 'bin'
    return f'terrestres/{uuid.uuid4().hex}.{ext}'


class ConfigTerrestre(models.Model):
    """Catálogo de serviços terrestres (Configurações). Nome, site, descrição e mídias."""
    name        = models.CharField('Nome', max_length=300, db_index=True)
    website     = models.CharField('Site', max_length=500, blank=True)
    description = models.TextField('Descrição', blank=True, default='')
    # False = criado só para um roteiro (não aparece no catálogo geral).
    is_global   = models.BooleanField('No catálogo geral', default=True, db_index=True)
...
class ConfigTerrestreMedia(models.Model):
    """Imagem ou vídeo de um serviço terrestre."""
    IMAGE = 'image'
    VIDEO = 'video'
    terrestre   = models.ForeignKey(ConfigTerrestre, on_delete=models.CASCADE, related_name='media')
    file        = models.FileField('Arquivo', upload_to=terrestre_media_path)
    kind        = models.CharField('Tipo', max_length=10, default=IMAGE)
    order       = models.PositiveIntegerField('Ordem', default=0)
    uploaded_at = models.DateTimeField('Enviado em', auto_now_add=True)
    ...
    def __str__(self):
        return f'{self.terrestre_id} · {self.kind}'

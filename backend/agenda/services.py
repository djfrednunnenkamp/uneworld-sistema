...
def collect_events(start: date, end: date):
    ...
    enrollments = (
        ...
        .select_related('passenger', 'passenger_list')
    )
    for en in enrollments:
        ...
        events.append({
            ...
            'type':     'deadline',
            'title':    f'Prazo: {who}',
            'subtitle': en.passenger_list.name,
...
def send_digest_email(user, days_ahead=14):
    ...
    if not user.email:
        return False
    today = date.today()
    events = collect_events(today, today + timedelta(days=days_ahead))
...
def send_reminder_email(user, days_before):
    ...
    if not user.email:
        return False
    today = date.today()
    events = collect_events(today, today + timedelta(days=days_before))

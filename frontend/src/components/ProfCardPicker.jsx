...
/**
 * ProfCardPicker — combobox com lista de carteiras do banco.
 * Permite digitar livremente (valor não precisa estar na lista).
 * Props: value (string), onChange (v: string) => void
 */
export default function ProfCardPicker({ value, onChange }) {
  const [profCards,    setProfCards]    = useState(cachedProfCards ?? [])
  ...
  // Carrega carteiras do banco
  useEffect(() => {
    if (cachedProfCards) return
    configApi.profCards().then(r => {
      cachedProfCards = r.data.map(v => v.name)
      setProfCards(cachedProfCards)
    }).catch(() => {})
  }, [])
  ...
  const filtered = useMemo(() => {
    ...
    return profCards.filter(v => !q || v.toLowerCase().includes(q)).slice(0, 60)
  }, [query, profCards])
    ...
    <div ref={wrapRef} style={{ position: 'relative' }}>
      <div style={{ position: 'relative' }}>
        <input
          ...
          onBlur={() => {
            if (query.trim()) {
              const exact = profCards.find(v => v.toLowerCase() === query.toLowerCase().trim())
          ...
          placeholder="Digite ou selecione a carteira…"
        ...
        <div style={drop}>
          {filtered.length === 0 ? (
            <p style={{ padding:'10px 14px', fontSize:13, color:'#94a3b8', margin:0 }}>
              {query ? `Nenhuma carteira encontrada. O texto "${query}" será salvo assim mesmo.` : 'Carregando…'}
            </p>
          ) : (
            <div ref={listRef}>

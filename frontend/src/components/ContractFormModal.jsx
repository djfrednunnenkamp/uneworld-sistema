...
export default function ContractFormModal({ contractId, onClose, onSaved, onPublish }) {
  ...
  useEffect(() => {
    ...
    contractsApi.get(contractId).then(async (r) => {
      ...
      setAppliedPlan(d.payment_plan_applied || null)
      if (ptype === 'a_vista') {
  ...
  // Efeito total de extras + descontos + desconto de comissão (badge do botão).
  const extrasNetUsd = adjustmentsTotalUsd - commissionDiscountUsd
  ...
  const applyPaymentPlan = (plan) => {
    ...
    if (n <= 0) {
      ...
      toast.success('Sugestão aplicada (pagamento à vista).')
      return
    }
    ...
    const rows = Array.from({ length: n }, (_, i) => ({
      ...
      value_brl: i === n - 1 ? round2(remaining - base * (n - 1)) : base,
      payment_method: method,
    }))
    setInstallmentsCount(n)
    setInstallments(rows)
    setAppliedPlan(snapshot)
    toast.success('Sugestão de pagamento aplicada.')
  }
  ...
  const buildPayload = () => {
    ...
    return {
      ...
      payment_plan_applied: appliedPlan,
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, marginBottom: 12, flexWrap: 'wrap' }}>
                ...
                <button type="button" onClick={() => setShowAdjustments(true)}
                  ...
                  {adjustments.length > 0 && (
                    <span style={{ fontSize: 11, fontWeight: 700, color: extrasNetUsd < 0 ? '#dc2626' : '#15803d' }}>
                      ({extrasNetUsd >= 0 ? '+' : '−'}{Math.abs(extrasNetUsd).toLocaleString('pt-BR', { minimumFractionDigits: 2 })} USD)
                    </span>
                  )}
                </button>
              </div>

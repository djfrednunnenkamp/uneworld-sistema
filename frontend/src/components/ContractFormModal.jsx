...
export default function ContractFormModal({ contractId, onClose, onSaved }) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      configApi.accommodations(), configApi.contractClauses(), configApi.operatingCompany(),
    ]).then(([ag, pax, ls, ac, cl, comp]) => {
      ...
      setClauses(cl.data)
      setPaymentMethods(comp.data.payment_methods ?? [])
      if (!isEdit) {
  ...
  useEffect(() => {
    ...
    contractsApi.get(contractId).then(r => {
      ...
      setForm({
        ...
        total_usd: d.total_usd ?? '', total_brl: d.total_brl ?? '', exchange_rate: d.exchange_rate ?? '',
  ...
  const listItems = useMemo(() => lists.map(l => ({ id: l.id, label: l.name, sublabel: l.start_date ? `Início: ${l.start_date.split('-').reverse().join('/')}` : '' })), [lists])
  ...
  const handleSelectList = (ids) => {
    ...
    setForm(f => ({
      ...f, passenger_list: id,
      package_name: f.package_name || l.name,
      departure_date: f.departure_date || l.start_date || '',
      departure_airport: f.departure_airport || (l.default_airport_data?.name ?? ''),
    }))
  }
  ...
  const save = async () => {
    ...
    const payload = {
      ...
      total_usd: form.total_usd || null, total_brl: form.total_brl || null,
            ...
            <div style={card}>
              ...
              <div style={{ display: 'flex', gap: 12 }}>
                <div style={{ flex: 1 }}>
                  ...
                  <input style={inp} value={form.reservation_number} onChange={set('reservation_number')} />
                </div>
                <div style={{ flex: 1 }}>
                  <label style={lbl}>Data desta contratação</label>
                  <input style={inp} type="date" value={form.contract_date} onChange={set('contract_date')} />
                </div>
                <div style={{ flex: 1 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <div style={{ display: 'flex', gap: 12 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <input style={inp} type="date" value={form.departure_date} onChange={set('departure_date')} />
                  </div>
                </div>
                <div style={{ display: 'flex', gap: 12 }}>
                  <div style={{ flex: 1 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <div style={{ display: 'flex', gap: 12 }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <input style={inp} type="number" step="0.01" value={form.total_usd} onChange={set('total_usd')} />
                  </div>
                  <div style={{ flex: 1 }}>
                    <label style={lbl}>Total em (BRL)</label>
                    <input style={inp} type="number" step="0.01" value={form.total_brl} onChange={set('total_brl')} />
                  </div>
                  <div style={{ flex: 1 }}>
            ...
            <div style={card}>
              ...
              <div style={{ marginBottom: 10 }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <input style={{ ...inp, flex: 1 }} type="date" value={entrada.due_date}
                    onChange={e => setEntrada(p => ({ ...p, due_date: e.target.value }))} />
              ...
              {installments.map((it, idx) => (
                <div key={idx} style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
                  ...
                  <input style={{ ...inp, flex: 1 }} type="date" value={it.due_date}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxHeight: 200, overflowY: 'auto' }}>
                  {clauses.map(c => (
                    <label key={c.id} style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
                      <input type="checkbox" checked={selectedClauses.includes(c.id)}
                      ...
                      {c.is_default && <span style={{ fontSize: 11, color: '#f59e0b' }}>★ padrão</span>}
                    </label>
                  ))}
                </div>
              )}
